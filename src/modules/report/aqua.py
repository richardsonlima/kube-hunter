...
def load_module():
    ...
    from default import TablesPrinted

    AQUA_LAMBDA_PUSH_URL = "https://qlyscbqwl7.execute-api.us-east-1.amazonaws.com/Prod/submit?token={token}"
    AQUA_LAMBDA_RESULTS_URL = "https://qlyscbqwl7.execute-api.us-east-1.amazonaws.com/Prod/result?token={token}"
    AQUA_RESULTS_URL = "https://kube-hunter.aquasec.com/report.html?token={token}"

    # making sure token is valid on first module load.
    if requests.get(AQUA_LAMBDA_RESULTS_URL.format(token=config.token)).status_code == 400:
        logging.error("\nToken is invalid\nGet your token from: https://kube-hunter.aquasec.com")
        sys.exit(1)
    ...
    class AquaReportSender(object):
        ...
        def execute(self):
            ...
            r = requests.put(AQUA_LAMBDA_PUSH_URL.format(token=config.token), json=self.event.report)
    ...
    @handler.subscribe(TablesPrinted)
    class AquaSendFullReport(object):
        ...
        def execute(self):
            ...
            logging.info("\nSee full report:\n{}".format(PrettyTable(["{}".format(AQUA_RESULTS_URL.format(token=config.token))], hrules=ALL)))
